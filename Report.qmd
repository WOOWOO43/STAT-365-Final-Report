...
Ultimately, what it seems to come down to is that if you're a profitable character, you get put in more issues. Outside of monetary value, from the data collected, there doesn't seem to be a lot of other contributing factors to a characters total appearances. We can also only generalize this to other X-Men comic book characters. We only looked at X-Men characters, so no other Marvel owned characters or characters outside of Marvel. We also cannot infer causality because there are definitely more factors that could contribute to a characters total appearances that we don't have data for, variables such as what writer wrote that character, amount of media appearances outside of the comics, and how many plots/storylines does that character appear in could all be contributing factors to a characters total appearance count. For those interested, these are definitely areas that need more investigating. 
