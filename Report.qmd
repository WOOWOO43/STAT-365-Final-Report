...
format: 
  pdf: 
    toc: false # Dont Include a table of contents
...
tabledata <- tabledata[order(tabledata$TotalIssues, decreasing = TRUE), c("Member", "TotalIssues", "Sex"), drop = FALSE]


color.male = which(tabledata$Sex == 'Male')
color.female = which(tabledata$Sex == 'Female')

kbl(tabledata, booktabs = T, row.names=FALSE) %>%
...
The partial F-test assesses whether the additional variable, TotalValue70s significantly improves the fit of the model.

The results indicate that Model 2, which includes TotalValue70s, provides a significantly better fit to the data compared to Model 1 with the predictors TotalValue60s, TotalValue90s, and TotalValue90s, as evidenced by (p-value = .052, df = (2,21), f-statistic = 4.22) However, this p-value is marginally above the conventional significance level of 0.05, suggesting that the improvement in fit may not be statistically significant at the 5% level but could be considered significant at a higher significance level.
